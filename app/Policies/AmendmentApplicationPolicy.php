...
class AmendmentApplicationPolicy
{
    /**
     ...
     * @return bool
     */
    public function amendment_view(User $user, AmendmentSociety $app)
    {
        return true;
     ...
     * @return bool
     */
    public function amendment_action(User $user, AmendmentSociety $app)
    {
        $userRole = strtolower($user->getRoleNames()->first()); // normalize to lowercase
        $appRole = strtolower($app->current_role);

        if (in_array($app->status, [2, 3, 5]) || $userRole  == "society") {
